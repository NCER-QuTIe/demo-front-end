...
<template>
  <div
    ...
    <Accordion class="mx-auto w-full h-min" @tab-close="() => (openTab = -1)"
      @tab-open="({ index }) => (openTab = index)">
      <AccordionPanel v-for="(d, ind) in data.slice(first, first + rows)" :key="d.name + ind" :value="ind"
        class="border-none">
        <AccordionHeader>
          <div class="test-listing flex justify-between items-baseline flex-grow gap-2 pr-4">
            ...
            <Button :outlined="openTab != ind" @click="$router.push('/test/' + d.id)" aria-label="Begin"
              icon="pi pi-play" label="დაწყება" class="start-button" />
          </div>
        </AccordionHeader>
        ...
        <AccordionContent pt:content:class="flex flex-col gap-4">
          ...
          <span class="actions flex gap-4 justify-between">
              ...
              <template #container="{ message, acceptCallback, rejectCallback }">
                <div class="rounded p-4">
                  ...
                  <div class="flex items-center gap-2 mt-4">
                    ...
                    <Button label="გაუქმება" outlined @click="rejectCallback" severity="secondary" size="small"
                      text></Button>
                  </div>
                </div>
              </template>
            ...
            <span class="flex gap-4 align-baseline">
              <Button @click="openPopup($event, d.id)" icon="pi pi-trash" label="წაშლა" severity="danger"
                aria-label="Visible" />
              <Button @click="edit($event, d)" icon="pi pi-pencil" label="რედაქტირება" severity="warn"
                aria-label="Visible" />
            </span>
          </span>
        </AccordionContent>
    ...
    <Paginator v-model:first="first" v-model:rows="rows" :totalRecords="data.length" :rowsPerPageOptions="[10, 20, 30]">
    </Paginator>
  </div>
</template>
